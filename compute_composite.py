#!/usr/bin/env python3
"""
Compute composite regime periods (regime5).

Classification rules combining SPY 10/20 EMA position with CNHNL breadth:
- Green:  SPY > 10EMA AND 10EMA > 20EMA AND CNHNL is Green
- Yellow: SPY > 10EMA AND 10EMA > 20EMA AND CNHNL is NOT Green
- Yellow: 10EMA > SPY > 20EMA (CNHNL irrelevant)
- Red:    SPY < both EMAs OR bearish EMA structure (20EMA >= 10EMA)

Generates regime periods and updates data.json with regimePeriods['regime5'],
regimeTrades['regime5'], and regimeStats['regime5'].
"""
...
DATA_JSON = 'data.json'
WARMUP_START = '2023-10-01'  # ~60 trading days before 2024-01-01 for 20EMA warmup
...
def classify_days(df, regime3_periods):
    """Classify each trading day using SPY 10/20 EMA + CNHNL rules.
    ...
    sorted_r3 = sorted(regime3_periods, key=lambda p: p['start'])
    daily_colors = []
    ...
    for i in range(len(df)):
        ...
        if e10 > e20 and c > e10:
            # Bullish structure, price above both EMAs — check CNHNL
            cnhnl = get_regime_color(date_str, sorted_r3)
            color = 'Green' if cnhnl == 'Green' else 'Yellow'
...
def update_data_json(periods, dry_run=False):
    """Write regime5 periods, trades, and stats to data.json."""
    print(f'\n-- Updating data.json --')
    ...
    # 2. Create regime5 trades by copying from regime1 and reclassifying
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute composite SPY 10/20 EMA + CNHNL regime periods (regime5)')
    ...
    # 1. Load regime3 (CNHNL) periods from data.json
    print('Loading regime3 (CNHNL) periods from data.json...')
    with open(DATA_JSON, 'r') as f:
        data = json.load(f)
    regime3_periods = data['regimePeriods'].get('regime3', [])
    if not regime3_periods:
        print('ERROR: No regime3 periods found in data.json. Run compute_cnhnl.py first.',
              file=sys.stderr)
        sys.exit(1)
    print(f'  Loaded {len(regime3_periods)} regime3 periods')

    # 2. Download SPY data
    df = download_spy(args.start, end_date)

    # 3. Classify each trading day
    daily_colors = classify_days(df, regime3_periods)
